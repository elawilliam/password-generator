...
if (
  !confirmNumers && !confirmUpperCaseLetters && !confirmLowerCaseLetters && !confirmSpecialCharacters){
  alert("Please select atleast one");
  return null;
  }

var employeePasswordOptions = {
...
console.log(employeePasswordOptions)
return employeePasswordOptions;

function createRandom (characters) {
  var randomCharacters = Math.floor(Math.random()*characters.length)
  var randomEl = characters[randomCharacters]
  return randomEl
}
...
if (options.confirmNumbers) {
  ...
  definiteCharacters.push(randomizer(numbers))
}

if (options.confirmUpperCaseLetters) {
  possibleCharacters = possibleCharacters.concat(upperCaseLetters)
  definiteCharacters.push(randomizer(upperCaseLetters))
}

if (options.confirmLowerCaseLetters) {
  possibleCharacters = possibleCharacters.concat(lowerCaseLetters)
  definiteCharacters.push(randomizer(lowerCaseLetters))
}

if (options.confirmSpecialCharacters) {
  possibleCharacters = possibleCharacters.concat(specialCharacters)
  definiteCharacters.push(randomizer(specialCharacters))
}

var size = options.employeeSelects - definiteCharacters.length

for (i=0; i < size; i++) {
  console.log (i,options,employeeSelects-size)
  definiteCharacters.push(randomizer(possibleCharacters))
}
}
